...
- Hint使用过程
  ...
  - 实现HintShardingAlgorithm接口，编写分库或分表路由策略
...
在读写分离结构中，为了避免主从同步数据延迟，可以采用强制路由走主库查询实时数据，使用hintManager.setMasterRouteOnly设置主库路由即可
...
![image-20200820112956976](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820112956976.png)

 Encrypt-JDBC将用户发起的SQL进行拦截，并通过SQL语法解析器进行解析，再依据用户传入的脱敏规则，找出需要脱敏的字段和所使用的加解密器，对目标字段进行加密处理后，再与底层数据库进行交互

- 脱敏规则

  脱敏配置主要分为四部分：数据源配置，加密器配置，脱敏表配置以及查询属性配置，其详情如下图所示

  ![image-20200820113248647](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820113248647.png)
...
- 脱敏处理流程
  ...
  ![image-20200820113451438](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820113451438.png)
...
- 分布式理论
  ...
  - CAP（强一致性）
    ...
    | 选项                | 描述                                                         |
    | ------------------- | ------------------------------------------------------------ |
    | Consistence         | 一致性。分布式环境中，数据在多个副本之间能够保持一致性       |
    | Availability        | 可用性。指系统提供的服务必须处于可用的状态                   |
    | Partition tolerance | 分区容错性。分布式系统在遇到任何网络分区故障的时候，任然能够对外提供满足一致性和可用性的服务 |

  - BASE（最终一致性）

    BASE 是指基本可用（Basically Available）、软状态（ Soft State）、终一致性（ Eventual Consistency）。它的核心思想是即使无法做到强一致性（CAP 就是强一致性），但可以采用适合的方式达到最终一致性

    - BA：指基本业务可用性，支持分区失败
    - S：表示柔性状态，也就是允许短时间内不同步
    - E：表示最终一致性，只要数据在一定时间内最终一致即可，不追求实时一致性
...
- 分布式事务模式
  ...
  - 2PC模式（强一致性） 
    ...
    - 阶段 1：准备阶段 
      ...
      协调者：向所有参与者发送事务内容，询问是否可以提交事务，并等待所有参与者答复

      参与者：执行事务操作（此时并不提交），并记录undo/redo日志。向协调者发送反馈，若执行成功，反馈yes；执行失败，返回no

    - 阶段 2：提交阶段 

      如果协调者收到了参与者的失败消息或等待超时，直接给每个参与者发送回滚(rollback)消息； 否则，发送提交(commit)消息
    ...
    - 单点故障问题：如果协调者出现故障，参与者将一直处于锁定状态
    - 数据一致性问题：在阶段 2 中，如果发生局部网络问题，一部分事务参与者收到了提交消息，另一部分事务参与者没收到提交消息，那么就导致了节点之间数据的不一致

  - 3PC模式（强一致性）

    3PC 三阶段提交，是两阶段提交的改进版本，在协调者和参与者中都引入超时机制。三阶段提交将两阶段的准备阶段拆分为 2 个阶段，插入了一个 preCommit 阶段，解决了原先在两阶段提交中，参与者在准备之后，由于协调者发生崩溃或错误，而导致参与者无法知晓处于长时间等待的问题

    - 阶段1：canCommit 

      协调者：向参与者发送commit请求

      参与者：可以提交就返回yes，否则返回no

    - 阶段2：preCommit 

      协调者：根据阶段1的反馈结果执行预提交事务或中断事务操作

      - 参与者均返回yes

        协调者：向所有参与者发出preCommit请求

        参与者：执行事务操作（此时不提交）；记录undo和redo日志；向协调者反馈ACK或no，并等待最终指令

      - 任何一个参与者返回no或等待超时

        协调者：向所有参与者发出abort请求

        参与者：收到abort请求或等待超时，均中断事务

    - 阶段3：do Commit 

      根据阶段2的反馈结果完成事务提交或中断

    优点：相比2PC模式，3PC模式降低了同步阻塞范围，在等待超时后协调者或参与者会中断事务（避免协调者单点故障），阶段3中如果协调者出现问题时，参与者会继续提交事务

  - XA模式（强一致性） 

    XA是基于两阶段提交协议的分布式事务规范。XA规范主要定义了全局事务管理器和局部资源管理器之间的接口。目前主流的关系型数据库产品都实现了XA接口

    ![image-20200820123735434](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820123735434.png)

    事务管理器用来保证所有的事务参与者都完成了2PC中的阶段1。MySQL在XA模式中扮演的是参与者的角色，而不是事务管理器

  - TCC模式（最终一致性） 

    TCC 是服务化的两阶段编程模型，其 Try、Conﬁrm、Cancel 3 个方法均由业务编码实现： 

    - try：负责资源的检查和预留
    - comfirm：执行业务
    - cancel：取消预留资源

    ![image-20200820124340936](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820124340936.png)
  ...
  - 消息队列模式（最终一致性）
    ...
    基于TCC模式。核心是将分布式事务拆分成本地事务进行处理，将需要分布式处理的任务通过消息日志的方式来异步执行

  - Saga模式（最终一致性）

    一个Saga事务是一个由多个短时间事务组成的长时间事务。在分布式场景下，我们把一个Saga分布式事务看做是一个由多个本地事务组成的事务，每个本地事务都有一个与之对应的补偿事务。在 Saga事务的执行过程中，如果某一步执行出现异常，Saga事务会被终止，同时会调用对应的补偿事务完成相关的恢复操作，这样保证Saga相关的本地事务要么都是执行成功，要么通过补偿恢复成事务执行之前的状态（自动反向补偿机制）

    Saga 事务基本协议如下：

    - 每个Saga事务由一些列幂等的有序子事务Ti组成

    - 每个Ti都有对应的幂等补偿动作Ci，补偿动作用于撤销Ti造成的结果

      Saga定义了两种补偿策略：

      - 向前恢复：发生失败进行重试，适用于必须要成功的场景
      - 向后恢复：发生错误后撤销掉之前成功的子事务，使得整个Saga的执行结果撤销
  ...
  - Seata框架
    ...
    - AT
      ...
      ![image-20200820131838955](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820131838955.png)
...
- 参数配置 
  ...
  Saga可以通过在项目的classpath中添加 saga.properties 来定制化Saga事务的配置项
...
- 配置中心
  ...
  动态化：支持数据源、表的分片及读写分离策略的动态切换

  - 配置中心数据结构

    配置中心在名为config的空间下，以YAML格式存储

    ```
    config    ├──authentication                            # Sharding-Proxy权限配置    ├──props                                     # 属性配置    ├──schema                                    # Schema配置    ├      ├──sharding_db                        # SchemaName配置    ├      ├      ├──datasource                  # 数据源配置    ├      ├      ├──rule                        # 数据分片规则配置    ├      ├──masterslave_db                     # SchemaName配置    ├      ├      ├──datasource                  # 数据源配置    ├      ├      ├──rule                        # 读写分离规则
    ```
    ...
    - conﬁg/schema/schemeName/datasource 
      ...
      ```
       ds_0:  dataSourceClassName: com.zaxxer.hikari.HikariDataSource    properties:      url: jdbc:mysql://127.0.0.1:3306/lagou1? serverTimezone=UTC&useSSL=false      password: root      username: root      maxPoolSize: 50      minPoolSize: 1  ds_1:    dataSourceClassName: com.zaxxer.hikari.HikariDataSource    properties:      url: jdbc:mysql://127.0.0.1:3306/lagou2? serverTimezone=UTC&useSSL=false      password: root      username: root      maxPoolSize: 50      minPoolSize: 1
    ...
    - conﬁg/schema/sharding_db/rule 
      ...
      ```
       tables:    b_order:      actualDataNodes: ds_$->{0..1}.b_order_$->{0..1}      databaseStrategy:
          inline:        shardingColumn: user_id          algorithmExpression: ds_$->{user_id % 2}    keyGenerator:        column: order_id    logicTable: b_order      tableStrategy:        inline:          shardingColumn: order_id          algorithmExpression: b_order_$->{order_id % 2}    b_order_item:      actualDataNodes: ds_$->{0..1}.b_order_item_$->{0..1}      databaseStrategy:        inline:          shardingColumn: user_id          algorithmExpression: ds_$->{user_id % 2}      keyGenerator:        column: order_item_id      logicTable: b_order_item      tableStrategy:        inline:          shardingColumn: order_id          algorithmExpression: b_order_item_$->{order_id % 2}
...
- 注册中心
  ...
  注册中心存放运行时的动态/临时状态数据。通过注册中心，可以提供熔断数据库访问程序对数据库的访问和禁用从库的访问的能力

  - 注册中心数据结构 

    注册中心在名为state的空间下，创建数据库访问对象运行节点，用于区分不同数据库访问实例。包括instances和datasources节点

    - state/instances 

      数据库访问对象运行实例信息，子节点是当前运行实例的标识。 运行实例标识由运行服务器 的IP地址和PID构成。运行实例标识均为临时节点，当实例上线时注册，下线时自动清理。 注册中心监控这些节点的变化来治理运行中实例对据库的访问等
...
- 应用性能监控
  ...
  ShardingSphere并不负责如何采集、存储以及展示应用性能监控的相关数据，而是将SQL解析与 SQL执行这两块数据分片的核心的相关信息发送至应用性能监控系统，并交由其处理。 换句话 说，ShardingSphere仅负责产生具有价值的数据，并通过标准协议递交至相关系统。 ShardingSphere可以通过两种方式对接应用性能监控系统
...
- 对应用程序完全透明，可直接当作MySQL使用
- 适用于任何兼容MySQL协议的客户端

![image-20200820143505994](C:\Users\x1850\AppData\Roaming\Typora\typora-user-images\image-20200820143505994.png)
...
- 解压缩后修改conf/server.yaml和以conﬁg-前缀开头的文件，进行分片规则、读写分离规则配置 编辑%SHARDING_PROXY_HOME%\conf\conﬁg-xxx.yaml 编辑%SHARDING_PROXY_HOME%\conf\server.yaml 
...
Mycat是一个实现了MySQL协议的server，其核心功能是分库分表和读写分离

###### 核心概念

- 逻辑库

  对数据进行分片处理之后，从原有的一个库，被切分为多个分片数据库，所有的分片数据库集群构成了 整个完整的数据库存储。Mycat在操作时，使用逻辑库来代表这个完整的数据库集群
...
- 分片表
  ...
  需要被切分到多个数据库的表，每一张表就是一个分片，每个分片都拥有一部分数据，所有分片构成完整的数据
...
- 全局表
  ...
  符合字典表特性的表将被定义为全局表，所有分片上都有这类表的备份（数据冗余解决表的join）
...
- 节点主机
  ...
  数据切分后，每个分片节点不一定都会独占一台机器，同一台机器上面可以有多个分片数据库，这样一个 或多个分片节点所在的机器就是节点主机，为了规避单节点主机并发数限制， 尽量将读写压力高的分片节点均衡的放在不同的节点主机dataHost
...
- firewall标签
  ...
  ```xml
  <firewall>  <!-- ip白名单 用户对应的可以访问的 ip 地址 -->  <whitehost>    <host host="127.0.0.*" user="root"/>    <host host="127.0.*" user="root"/>
      <host host="127.*" user="root"/>    <host host="1*7.*" user="root"/>  </whitehost>  <!-- 黑名单允许的 权限 后面为默认 -->  <blacklist check="true">    <property name="selelctAllow">false</property>    <property name="selelctIntoAllow">false</property>    <property name="updateAllow">false</property>    <property name="insertAllow">false</property>    <property name="deletetAllow">false</property>    <property name="dropAllow">false</property>  </blacklist> </firewall>
  
  ```

- 全局序列号

  Mycat 提供了全局 sequence，并且提供了包含本地配置和数据库配置等多种实现方式
...
管理Mycat的逻辑库、逻辑表、分片节点、主机等信息

- schema标签

  定义逻辑库，可以有多个逻辑库，每个逻辑库都有自己的相关配置
  ...
  | sqlMaxLimit    | Integer    | (1)      | 查询返回的记录数限制 |
...
- table标签
  ...
  | needAddLimit  | Boolean | (0..1)   | 是否为查询SQL自动添加limit限制 |
...
- dataNode标签
  ...
  | -------- | ------------------------------ |
  | name     | 定义数据节点的名字，唯一       |
  | dataHost | 定义该分片节点属于哪个分片主机 |
  | database | 定义该分片节点属于哪个库       |

- dataHost标签

  定义了具体的数据库实例、读写分 离配置和心跳语句
...
- heartbeat标签
  ...
  ```xml
  <dataHost>    
      <heartbeat>select user()</heartbeat> </dataHost>
  ```

- writeHost和readHost标签

  writeHost和readHost标签都指定后端数据库的相关配置，用于实例化后端连接池

  ```xml
  <dataHost name="lg_edu_order_2" maxCon="100" minCon="10" balance="0" writeType="0" dbType="mysql" dbDriver="native" switchType="1" slaveThreshold="100">    <heartbeat>select user()</heartbeat>    <writeHost host="M1" url="192.168.95.133:3306" user="root" password="1234">    </writeHost> </dataHost>
  ```
...
```xml
<tableRule name="c_order_rule">    <rule>        <columns>user_id</columns>        <algorithm>partitionByOrderFunc</algorithm>    </rule> </tableRule>
```
...
| name      | 指定规则名称，唯一                                     |
| columns   | 指定要拆分的列名字                                     |
| algorithm | 使用function标签中的name属性，连接表规则与具体路由算法 |
...
```xml
<function name="partitionByOrderFunc" class="io.mycat.route.function.PartitionByMod">    <property name="count">2</property> </function>
```
...
- 分库分表配置rule.xml
  ...
  <mycat:rule xmlns:mycat="http://io.mycat/">    
   ...
      <!-- 路由函数定义 -->    
      <function name="partitionByOrderFunc" class="io.mycat.route.function.PartitionByMod">        <property name="count">2</property>    
      </function> 
      
   </mycat:rule>
  ```
  ...
  - 哈希类：Hash固定分片、日期范围Hash分片、截取数字Hash求模范围分片、截取数字Hash分 片、一致性Hash分片 
  - 取模类：取模分片、取模范围分片、范围求模分片 
  - 其他类：枚举分片、范围约定分片、应用指定分片、冷热数据分片

- 读写分离配置schema.xml

  前置条件：已搭建MySQL主从架构，并实现主从复制

  ```
  <dataHost name="localhost1" maxCon="1000" minCon="10" balance="1" writeType="0" dbType="mysql" dbDriver="native">     <heartbeat>select user()</heartbeat>    <!-- can have multi write hosts -->    <writeHost host="M1" url="localhost:3306" user="root" password="123456">         <readHost host="S1" url="localhost:3307" user="root" password="123456"             weight="1" />    </writeHost> </dataHost>
  ```
...
- 强制路由
  ...
  ```
  强制走从: /*!mycat:db_type=slave*/ select * from travelrecord //有效 /*#mycat:db_type=slave*/ select * from travelrecord 强制走写: /*!mycat:db_type=master*/ select * from travelrecord //有效 /*#mycat:db_type=slave*/ select * from travelrecord 
...
- Mycat数据库事务
  ...
  Mycat 目前没有跨分片的事务强一致性支持，单库内部可以保证事务的完整性，如果跨库事务， 在执行的时候任何分片出错，可以保证所有分片回滚，但是一旦应用发起 commit 指令，无法保证所有分片都成功，考虑到某个分片挂的可能性不大所以称为弱 XA

- XA事务

  Mycat 实现 XA 标准分布式事务，Mycat 作为XA 事务协调者角色，即使在事务过程中 Mycat 宕机， 由于 Mycat 会记录事务日志，所以 Mycat 恢复后会进行事务的恢复善后处理工作。考虑到分布式事务的性能开销比较大，所以只推荐在全局表的事务以及其他一些对一致性要 求比较高的场景
...
- 保证Repeatable Read 
  ...
  mycat 有一个特性，就是开事务之后，如果不运行 update/delete/select for update 等更新类语句 SQL 的话，不会将当前连接与当前 session 绑定。这样做的好处是是可以保证连接可以大限度的复用，提升性能。可通过server.xml中的system属性strictTxIsolation=true关闭该特性。以保证repeatable read
